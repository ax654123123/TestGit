...
{
    expect_file_size = [[[(NSHTTPURLResponse*)response allHeaderFields] objectForKey:@"Content-Length"] integerValue];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
	...
	if (moLoadingDataFile) {
        ...
        // yh
        NSString* readyFilename = nil;
        NSError* e=nil;
        HJMOFileCache* fileCache = objManager.fileCache;
        NSString* loadingFile = [fileCache loadingFilePathForOid:oid];
        NSDictionary* dict = [[NSFileManager defaultManager] attributesOfItemAtPath:loadingFile error:&e];
        NSNumber* size = [dict objectForKey:NSFileSize];
        if ((NSInteger)size.unsignedIntegerValue >= expect_file_size) {
            readyFilename = [self.objManager.fileCache loadingFinishedForOid:oid];
        }
        // yh
